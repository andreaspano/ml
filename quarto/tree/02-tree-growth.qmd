...
## Growth 
### Chose your next split variable
...
- Growing a tree is about mapping the data onto a _split domain_

    - The data lives in a feature space. Two explanatory variables $x_1$ and $x_2$ live i a $2d$ plane

- Splitting means: partitioning that space.

    - A tree recursively partitions this feature space into rectangular regions (or hyper-rectangles in higher dimensions)

    - After several splits, we have carved up the space into non-overlapping regions.

- Mapping data onto the split domain.

    - Every observation falls into exactly one of these regions.

    - Those regions correspond to the leaves of the tree.

- Within each leaf, you assign a predicted class (e.g., majority class in the training data for that region).
